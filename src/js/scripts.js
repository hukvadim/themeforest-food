 ...
 */
const toastBtns = document.querySelectorAll('.js-call-toast');
if (toastBtns.length > 0) {
	toastBtns.forEach(btnNode => {
		const forToast = document.getElementById(btnNode.dataset.forToast);
		btnNode.addEventListener('click', () => {
			const toast = new bootstrap.Toast(forToast);
			toast.show();
		})
	})
}
